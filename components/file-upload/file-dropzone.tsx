...
export function FileDropzone({
  ...
  return (
    <div
      {...mergedProps}
      className={cn(
        "relative flex min-h-[200px] cursor-pointer touch-manipulation flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 transition-colors select-none",
    ...
    >
      ...
      <div className="pointer-events-none flex flex-col items-center gap-4 text-center select-none">
        <div
          className={cn(
            ...
            isDragReject && "text-destructive-foreground bg-destructive",
        ...
        {maxFiles === 1 && (
          <p className="text-xs text-muted-foreground">
            Maximum file size:{" "}
            {maxSize ? `${(maxSize / 1024 / 1024).toFixed(0)}MB` : "N/A"}
          </p>
        )}
      </div>
    </div>
