...
import {
  ...
  getOrphanedFiles,
  hardDeleteFile,
} from "@/lib/file-upload/file-crud"
  ...
  try {
    ...
    for (const file of expiredFiles) {
      try {
        ...
        console.log(
          `✅ Deleted expired file: ${file.originalFilename} (${file.id})`
        )
      } catch (error: unknown) {
        result.failedCount++
        result.errors.push({
  ...
  try {
    ...
    for (const file of orphanedFiles) {
      try {
        ...
        console.log(
          `✅ Deleted orphaned file: ${file.originalFilename} (${file.id})`
        )
      } catch (error: unknown) {
        result.failedCount++
        result.errors.push({
...
export async function runAllCleanup(): Promise<{
  ...
  console.log(
    `✅ Cleanup complete: ${totalDeleted} files deleted, ${totalFailed} failed`
  )
