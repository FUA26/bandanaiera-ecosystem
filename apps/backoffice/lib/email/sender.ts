...
export async function sendTemplate(
  ...
  console.log(
    `📧 [sendTemplate] Called with template: ${templateName}, to: ${options.to}`
  )
  console.log(`📧 [sendTemplate] RESEND_API_KEY exists:`, !!env.RESEND_API_KEY)
  console.log(
    `📧 [sendTemplate] RESEND_API_KEY value:`,
    env.RESEND_API_KEY
      ? `${env.RESEND_API_KEY.substring(0, 8)}...`
      : "undefined/empty"
  )
  ...
  if (!env.RESEND_API_KEY) {
    if (env.NODE_ENV === "development") {
      console.log(
        "📧 [Email Mock] RESEND_API_KEY not set - skipping email send"
      )
  ...
  try {
    ...
    console.log(
      `📧 [sendTemplate] Template loaded, size: ${templateHtml.length} chars`
    )

    // Render with data
    const renderedHtml = renderTemplate(templateHtml, options.data)
    console.log(
      `📧 [sendTemplate] Rendered HTML size: ${renderedHtml.length} chars`
    )
