 ...
 */
export async function notifyAgentTicketAssigned(
  ticketId: string,
  agentId: string
) {
...
export async function notifyCustomerTicketUpdate(
  ...
  if (ticket.userId) {
    console.log(
      `[Notification] Ticket update for user ${ticket.userId}: ${updateType}`
    )
 ...
 */
export async function notifyCustomerTicketCreated(
  ticketId: string,
  ticketNumber: string
) {
  const ticket = await prisma.ticket.findUnique({
