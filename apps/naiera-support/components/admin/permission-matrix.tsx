...
function getPermissionDescription(permission: Permission): string {
  const descriptions: Record<string, string> = {
    // User permissions
    USER_READ_OWN: "View your own profile and account information",
    USER_READ_ANY: "View any user's profile and account information",
    USER_UPDATE_OWN: "Edit your own profile and account information",
    USER_UPDATE_ANY: "Edit any user's profile and account information",
    USER_DELETE_OWN: "Delete your own account",
    USER_DELETE_ANY: "Delete any user's account",
    USER_CREATE: "Create new user accounts",

    // Content permissions
    CONTENT_READ_OWN: "View content you created",
    CONTENT_READ_ANY: "View any content regardless of owner",
    CONTENT_CREATE: "Create new content",
    CONTENT_UPDATE_OWN: "Edit content you created",
    CONTENT_UPDATE_ANY: "Edit any content regardless of owner",
    CONTENT_DELETE_OWN: "Delete content you created",
    CONTENT_DELETE_ANY: "Delete any content regardless of owner",
    CONTENT_PUBLISH: "Publish content to live site",

    // Settings permissions
    SETTINGS_READ: "View application settings",
    SETTINGS_UPDATE: "Modify application settings",
    ...
    // Admin permissions
    ADMIN_PANEL_ACCESS: "Access the admin panel",
    ADMIN_USERS_MANAGE: "Manage user accounts and roles",
    ADMIN_ROLES_MANAGE: "Create and modify roles and permissions",
    ADMIN_PERMISSIONS_MANAGE: "Create and modify system permissions",
  }

  return (
    descriptions[permission] ||
    `Controls access to ${permission.toLowerCase().replace(/_/g, " ")}`
  )
}
          ...
          >
            ...
            <div className="space-y-2">
              {category.permissions.map((permission) => (
                <div
                  key={permission}
                  className="flex items-center gap-3 rounded-md px-2 py-1 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring has-[:focus-visible]:ring-offset-2"
                >
                  <Checkbox
                    id={permission}
