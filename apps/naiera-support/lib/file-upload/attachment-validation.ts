...
export const attachmentFileSchema = z.object({
  ...
  size: z
    ...
    .max(
      MAX_ATTACHMENT_SIZE,
      `File size must be less than ${formatFileSize(MAX_ATTACHMENT_SIZE)}`
    ),
  type: z.enum([...ALLOWED_ATTACHMENT_TYPES], {
    errorMap: () => ({
      message:
        "Invalid file type. Only images and PDF/Office documents allowed.",
    }),
  }),
})
