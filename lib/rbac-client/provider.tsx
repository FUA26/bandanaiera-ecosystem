...
export function PermissionProvider({
  ...
  const [permissions, setPermissions] = useState<UserPermissionsContext | null>(
    initialPermissions
  )
  ...
  if (!refreshRef.current) {
    refreshRef.current = async () => {
      ...
      try {
        ...
        const { enableCache: currentEnableCache, cacheTTL: currentCacheTTL } =
          configRef.current
   ...
   */
  const canRef = useRef<
    ((requiredPermissions: Permission[]) => boolean) | null
  >(null)
  if (!canRef.current) {
    canRef.current = (requiredPermissions: Permission[]): boolean => {
      ...
      return requiredPermissions.some((p) =>
        currentPermissions.permissions.includes(p)
      )
    }
  }
  ...
  useEffect(() => {
    ...
    if (currentPollingInterval !== null && currentPollingInterval > 0) {
      console.log(
        `[PermissionProvider] Setting up polling every ${currentPollingInterval}ms`
      )
