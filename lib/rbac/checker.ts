...
export function hasPermission(
  ...
  const results = requiredPermissions.map((requiredPerm) => {
    ...
    if (requiredPerm.endsWith("_OWN") && currentUserId && resourceOwnerId) {
      ...
      if (
        anyPermission &&
        normalizedPermissions.has(anyPermission as Permission)
      ) {
      ...
      if (
        ...
        normalizedPermissions.has(requiredPerm)
      ) {
        return { allowed: true, grantedBy: requiredPerm }
      }
    }
 ...
 */
export function getAnyPermission(
 ...
 */
export function isUserPermissionsContext(
  value: unknown
): value is UserPermissionsContext {
